...
import CashSound from '../../../../../../public/sounds/cash-sound.mp3'
...
export default function CrashForm({
  ...
  const moneySoundRef = useRef(new Audio('/caminho/do/seu/audio.mp3'));
  ...
  const doubleAmount = () => {
    moneySoundRef.current.pause();
    moneySoundRef.current.currentTime = 0;
    updateAmount(formatBRLCurrency(transaction.amount * 2));
    moneySoundRef.current.play();
  };
  ...
  return (
    <div className="bg-black border border-gray-600 bg-opacity-20 border-opacity-20 crash-form w-full h-45 md:w-1/2 flex rounded-md p-3 relative">
      ...
      >
        ...
        <audio ref={moneySoundRef} preload="auto">
          <source src={CashSound} type="audio/mp3" />
